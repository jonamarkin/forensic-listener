...
CREATE TABLE IF NOT EXISTS case_addresses (
    ...
    role    TEXT NOT NULL DEFAULT 'subject'
            CHECK (role IN ('subject', 'counterparty', 'watch', 'entity', 'contract')),
...
CREATE TABLE IF NOT EXISTS flag_triage (
    flag_id       INTEGER PRIMARY KEY REFERENCES forensic_flags(id) ON DELETE CASCADE,
    status        TEXT NOT NULL DEFAULT 'new'
                  CHECK (status IN ('new', 'reviewing', 'escalated', 'dismissed', 'resolved')),
